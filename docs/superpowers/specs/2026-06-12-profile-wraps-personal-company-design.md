...
`_ensure_proxy` already reuses a healthy proxy on the port. Because each profile has a **distinct port**, a `company` proxy (8788-ish, both aperture upstreams) and a `personal` proxy (8787, default upstreams) coexist without contaminating each other, and the second agent of a profile attaches to the already-running profile proxy. The proxy's health/manifest records the upstreams; if a proxy on the target port is running with a *different* upstream set than the profile requires, headroom restarts it (idle) or warns (active) — reuse the existing version-restart machinery in `_ensure_proxy` as the precedent.
