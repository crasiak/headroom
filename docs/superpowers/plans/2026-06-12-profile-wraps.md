...
def test_select_profile_precedence(monkeypatch):
    ...
    assert (
        select_profile_name(
            flag="company", env={"HEADROOM_PROFILE": "x"}, config_default="personal"
        )
        == "company"
    )
    assert (
        select_profile_name(flag=None, env={"HEADROOM_PROFILE": "envp"}, config_default="personal")
        == "envp"
    )
...
def select_profile_name(
    *, flag: str | None, env: dict[str, str], config_default: str | None
) -> str:
...
def resolve_profile(name: str, *, profiles_path: Path | None = None) -> ResolvedProfile:
    ...
    port = (
        int(profile["port"])
        if "port" in profile
        else _profile_port(name, default_profile=default_profile)
    )
...
def test_health_payload_exposes_bedrock_api_url():
    proxy = HeadroomProxy(
        ProxyConfig(
            bedrock_base_url="https://ap/bedrock", cache_enabled=False, rate_limit_enabled=False
        )
    )
...
```python
if bedrock_api_url:
    running_bedrock_url = _normalize_proxy_api_url(running_config.get("bedrock_api_url"))
...
def _resolve_claude_profile(*, flag: str | None, bedrock_base_url: str | None):
    ...
    if bedrock_base_url:  # explicit flag overrides the profile's bedrock url
        rp = ResolvedProfile(
            ...
            port=rp.port,
            bedrock_base_url=bedrock_base_url,
            claude_env=rp.claude_env,
            openai_upstream=rp.openai_upstream,
