...
def isolated_native_home(tmp_path, monkeypatch):
    """Profile wiring tests must not inherit the invoking native account."""
...
def test_codex_profile_mode_port_flag_wins(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    with runner.isolated_filesystem():
        result = runner.invoke(codex, ["--profile", "company", "--prepare-only", "--port", "9999"])
    assert result.exit_code == 0, result.output
...
def test_scrub_ambient_bedrock_env_bedrock_profile_untouched():
    ...
    env = _ambient_bedrock_env()
    _scrub_ambient_bedrock_env(env, profile_name="company", bedrock_base_url="https://ap/bedrock")
